...
const CryptoLoanBorrowed = Mongoose.model("cryptoLoanBorrowed");
const Repayment = Mongoose.model("loanRepayment");
...
* @returns {Promise<CryptoLoanBorrowed>}
*/
...
const borrowedCreateServices = async (coinBody) => {
    ...
    return CryptoLoanBorrowed.create(coinBody).then(async (loanBorrow) => {
                        ...
                        await userWalletCollateralCoin.save()
                            .then(
                                async (userWalletCollateral) => {
                                    ...
                                    await commonHelper.cryptoLoanCollateralBalance(userId, collateralCurrencyId, newAmount, oldAmount, cryptoLoanId, 'Crypto loan collateral asset')
                        ...
                        await userWalletBorrowCoin.save()
                            .then(
                                async (userWalletBorrow) => {
                                    ...
                                    await commonHelper.cryptoLoanBorrowBalance(userId, borrowCurrencyId, newAmount, oldAmount, cryptoLoanId, 'Crypto loan borrow asset')
                                }).catch((err) => console.log(err));
        ...
        await commonHelper.loanActivityLogs(userId, " ", " ", cryptoLoanId, 'Crypto Loan Borrowed', `Crypto loan ${borrowAmt} ${borrowCoin} borrowed successfully`)
 ...
 * @param {empty} 
 * @returns {Promise<LoanBorrowDetails>} ArrayOfObject
 */
 ...
 * @returns {Promise<RepaymentDetails>}
 */
        ...
        if (!orderRepayDetails) {
            ...
            return Repayment.create(orderData).then(async (repay) => {
                    ...
                    UserWallet.findOne(
                        ...
                        .then(async (userWalletCollateralCoin) => {
                            ...
                            if (userWalletCollateralCoin) {
                                ...
                                Object.assign(userWalletCollateralCoin);
                                await userWalletCollateralCoin.save();
                                await commonHelper.cryptoLoanCollateralBalance(userId, collateralCurrencyId, newAmount, oldAmount, repay._id, 'Crypto loan repaid accuring interest collateral coin')
                                walletData.push(userWalletCollateralCoin);
                                // return walletData;
                            }
                        }).catch((err) => console.log(err))
                    ...
                    UserWallet.findOne(
                        ...
                        .then(async (userWalletBorrowCoin) => {
                            ...
                            if (userWalletBorrowCoin) {
                                ...
                                // .then(async() => {
                                // }).catch((err) => console.log(err));
                                await commonHelper.cryptoLoanBorrowBalance(userId, borrowCurrencyId, newAmount, oldAmount, repay._id, 'Crypto loan repaid accuring interest borrow coin')
                                walletData.push(userWalletBorrowCoin);
                            }
                        }).catch((err) => console.log(err))
                );
                if (repayData) {
                    ...
                    await commonHelper.loanActivityLogs(userId, " ", " ", repay._id, 'Crypto Loan Repaid', `Crypto loan due amount ${parseFloat(repay.due_detail[0].due_paid_amount).toFixed(8)} is repaid`)
                    return repayData;
                }
                ...
                UserWallet.findOne(
                    ...
                    .then(async (userWalletCollateralCoin) => {
                        ...
                        if (userWalletCollateralCoin) {
                            ...
                            // .then(async() => {
                            // }).catch((err) => console.log(err));
                            await commonHelper.cryptoLoanCollateralBalance(userId, collateralCurrencyId, newAmount, oldAmount, orderRepayDetails._id, 'Crypto loan repaid accuring interest collateral coin');
                        }
                    }).catch((err) => console.log(err))
            );
            borrowCurrencyId && (
                UserWallet.findOne(
                    ...
                    { amount: 1, _id: 1, userId: 1, currencyId: 1, cryptoLoanHold: 1, cryptoLoanAmount: 1 })
                    .then(async (userWalletBorrowCoin) => {
                        let oldAmount = userWalletBorrowCoin.amount;
                        if (userWalletBorrowCoin) {
                            ...
                            // .then(async() => {
                            // }).catch((err) => console.log(err));
                            await commonHelper.cryptoLoanBorrowBalance(userId, borrowCurrencyId, newAmount, oldAmount, orderRepayDetails._id, 'Crypto loan repaid accuring interest borrow coin');
                        }
                    }).catch((err) => console.log(err))
            );
            if (orderRepayDetails) {
            ...
            if (repayData) {
                ...
                repayAmtSub = repayData.remainingPrinciple - orderData.repaymentAmount;
                ...
                Object.assign(repayData);
                await repayData.save();
                await commonHelper.loanActivityLogs(userId, " ", " ", orderRepayDetails._id, 'Crypto Loan Repaid', `Crypto loan due amount ${parseFloat(orderData.due_detail[0].due_paid_amount).toFixed(6)} is repaid`);
                return repayData;
            }
            repayProcessing = false;
