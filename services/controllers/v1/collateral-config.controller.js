...
const { collateralServices } = require("../../services");
...
//** get all currency list coins */
const getCurrencyList = async (req, res) => {
    try {
        const currencyList = await collateralServices.getCurrencyListServices();
        return successResponse(req, res, currencyList.data);
    } catch (error) {
        return errorResponse(req, res, error.message);
    }
}
